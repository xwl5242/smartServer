...
class SHA1:
    ...
    def sha1(self, token, timestamp, nonce, encrypt):
        ...
        try:
            ...
            sort_list.sort()
            sha = hashlib.sha1()
            sha.update(to_utf8_bytes("".join(sort_list)))
...
class Prpcrypt(object):
    ...
    def encrypt(self, text, app_id):
        ...
        # noinspection PyBroadException
        try:
            cipher_text = cryptor.encrypt(text_bytes)
            # 使用BASE64对加密后的字符串进行编码
            return error.WXBizMsgCrypt_OK, utf8_bytes_to_str(base64.b64encode(cipher_text))
        except Exception:
            logger.exception('wechat encryption/decryption error')
...
class WXBizMsgCrypt(object):
    ...
    def __init__(self, s_token, s_encoding_aes_key, s_app_id):
        # noinspection PyBroadException
        try:
    ...
    def encrypt_msg(self, s_reply_msg, s_nonce, timestamp=None):
        ...
        ret, encrypt = pc.encrypt(s_reply_msg, self.app_id)
        if ret != 0:
            return ret, None
        ...
        # 生成安全签名
        sha1 = SHA1()
        ret, signature = sha1.sha1(self.token, timestamp, s_nonce, encrypt)
        if ret != 0:
            return ret, None
        xml_parse = XMLParse()
    ...
    def decrypt_msg(self, s_post_data, s_msg_signature, s_time_stamp, s_nonce):
        ...
        # 验证安全签名
        xml_parse = XMLParse()
        ret, encrypt, to_user_name = xml_parse.extract(s_post_data)
        if ret != 0:
            return ret, None
        sha1 = SHA1()
        ret, signature = sha1.sha1(self.token, s_time_stamp, s_nonce, encrypt)
        if ret != 0:
            return ret, None
